...
def test_code(noise_perc, num_points, trial_id, poly=2, tfinal=4, alpha=0.2):
    # 1) Simulate clean data, make list
    ...
    poly_feature_ob = PolyFeatureMatrix(poly)
    ...
    # 5) Discover conservation laws w/ enz
    ...
    next_best_relationship = reduced_relationship_features[1]

    if None in best_relationship:
        best_relationship.remove(None)

    # 6) Perform first check: did we find the 2 laws?
    if set(best_relationship) != set(['[E1]', '[AE1]']) and set(best_relationship) != set(['[E2]', '[AE2]']):
        return False
    if set(next_best_relationship) != set(['[E1]', '[AE1]']) and set(next_best_relationship) != set(['[E2]', '[AE2]']):
        return False
    ...
    return True # If made it here, then correct rels
...
def main_loop(output_file, poly):
    ...
    for (noise, trial), group in grouped:
        ...
        processed_results.append({
            "Noise": noise,
            "Trial": trial,
            "Max_Frequency_False": max_frequency
        })
...
def main(output_file, poly):
    ...
    trials = 3 # Number of trials per noise level
